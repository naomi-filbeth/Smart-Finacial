...
class DebtorsProvider with ChangeNotifier {
  ...
  final String _baseUrl = 'http://192.168.1.106:8000/api'; // Update to 'http://10.0.2.2:8000/api' for Android emulator
  ...
  Future<void> addDebtor(String name, double balance, {String? email, String? phone, String? address}) async {
    try {
      final token = _authProvider!.token;
      if (token == null) {
      ...
      if (response.statusCode == 201) {
        ...
        if (newDebtor['balance'] is String) {
          newDebtor['balance'] = double.tryParse(newDebtor['balance'] as String) ?? balance;
        }
  ...
  Future<void> updateDebtor(int id, double balance, {String? email, String? phone, String? address}) async {
    try {
      final token = _authProvider!.token;
      if (token == null) {
        throw Exception('No authentication token available');
      ...
      if (response.statusCode == 200) {
        ...
        if (index != -1) {
          ...
          if (updatedDebtor['balance'] is String) {
            updatedDebtor['balance'] = double.tryParse(updatedDebtor['balance'] as String) ?? balance;
          }
